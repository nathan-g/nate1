...
CAPTURE.init = function() {
    ...
    CAPTURE.createSession = function(token, fromCookie) {
        ...
        if (!CAPTURE.util.getCookie('janrainProfile')){
            CAPTURE.getUserData(accessToken);
        } else {
            // Make callback as if finishCreateSession was called
            (cfg.callbacks.getuserDataFinish || noop)()
        }
    };

    CAPTURE.getUserData = function(accessToken) {
        this.util.domReady(win, cfg.callbacks.getUserDataStart || noop);
        var attrib = JSON.stringify(['uuid',
                                     'photos',
        ...
        var srcUrl = [
            ...
            '&attributes=', encodeURIComponent(attrib),
    ...
    CAPTURE.userUpdatedData = function(profileData) {
        // Refresh user's data
        var accessToken = CAPTURE.util.getCookie('janrainToken');
        if (accessToken) {
            CAPTURE.getUserData(accessToken);
        }
    ...
    CAPTURE.userStartedRegistration = function() {
        CAPTURE.util.domReady(win,
                              cfg.callbacks.userStartedRegistration || noop);
    };
    ...
    CAPTURE.finishCreateSession = function(userData) {
        ...
        if (userData.transaction_state &&
            ...
            if (userData.transaction_state.capture.password_recover) {
                CAPTURE.util.domReady(win, cfg.callbacks.recoverPasswordComplete || noop);
            }
            if (userData.transaction_state.capture.action &&
                (userData.transaction_state.capture.action === 'finish_third_party' ||
                 userData.transaction_state.capture.action === 'legacy_register')) {
                    (cfg.callbacks.userRegistered || noop)(userData);
                }
        }
        ...
        // setting the diff to 5 minutes, i.e. call Omniture: handleUserRegistrationComplete
        // if the user was created in the last 5 minutes.
        var recent_registration_millis = 1000 * 60 * 5;
        var user_created_millis;

        try {
            user_created_millis = Date.parse(userData.result.created);

            // see if the user registered within the past few minutes
            if (new Date().getTime() - user_created_millis < recent_registration_millis) {
                OMNITURE_AI.handleUserRegistrationComplete()
            }
            else {
                console.log ("user registered more than " + user_created_millis + " millis ago.")
            }

        }
        catch (err) {
            console.log ("unable to get user's created date: " + err.description)
        }
        ///////////////////////////////////////////////////////////////////////////////////
